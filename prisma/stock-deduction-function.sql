-- =====================================================
-- create_order_with_stock_deduction (v2 - per-size stock)
-- Atomic order creation with per-size stock validation & deduction
-- =====================================================
-- This function handles the entire order flow in a single
-- PostgreSQL transaction:
--   1. Upserts the customer
--   2. Validates per-size stock with SELECT FOR UPDATE
--   3. Creates the order
--   4. Creates order items (with size)
--   5. Deducts stock from the specific size
--   6. Recalculates total stock from sizeStock
--   7. Auto-sets inStock = false when total stock reaches 0
--
-- Returns JSON on success or error for insufficient stock.
-- =====================================================
...
CREATE OR REPLACE FUNCTION create_order_with_stock_deduction(
  ...
  p_order_notes TEXT,
  p_total INT,
  p_items JSONB  -- Array of {productId, name, image, quantity, price, size}
) RETURNS JSON
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    ...
    -- Deduct stock from the specific size using jsonb_set
    UPDATE products
    ...
    -- Auto-set inStock = false when total stock reaches 0
